...
class LayoutDetectionModule(nn.Module):
    def __init__(self, input_dim, num_classes=4, max_bbox = 40):
        super(LayoutDetectionModule, self).__init__()
        self.max_bbox = max_bbox
        self.fc1 = nn.Linear(input_dim, 256)
        self.fc2 = nn.Linear(256, 64)
        self.fc_bbox = nn.Linear(64, max_bbox * 4)
        self.fc_class = nn.Linear(64, max_bbox * num_classes)
    ...
    def forward(self, x):
        ...
        bbox_output = bbox_output.view(x.size(0), self.max_bbox, 4)
        class_output = class_output.view(x.size(0), self.max_bbox, 2)
